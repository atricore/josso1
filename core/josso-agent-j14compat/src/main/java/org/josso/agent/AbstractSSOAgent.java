...
public abstract class AbstractSSOAgent implements SSOAgent {
    ...
    public SSOPartnerAppConfig getPartnerAppConfig(String vhost, String contextPath) {
        ...
        List papps = _cfg.getSsoPartnerApps();
        for (int i = 0; i < papps.size(); i++) {

            SSOPartnerAppConfig ssoPartnerAppConfig = (SSOPartnerAppConfig) papps.get(i);
    ...
    protected void localSessionDestroyedEvent(LocalSession session) {
        ...
        synchronized (reverse) {
            ssoId = (String) reverse.get(session.getId());
        }
        if (ssoId == null)
            return;
    ...
    protected void associateLocalSession(String ssoId, LocalSession localSession) {
        ...
        synchronized (reverse) {
            reverse.put(localSession.getId(), ssoId);
        }
    ...
    protected void deregister(String ssoId) {
        ...
        for (int i = 0; i < localSessions.length; i++) {
            ...
            synchronized (reverse) {
                reverse.remove(localSessions[i].getId());
            }
